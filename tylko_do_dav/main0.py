...
def extract_single_frame(cap, frame_index, total_frames, orientation, frame_rotation):
    ...
    try:
        ...
        if not ret:
            # Szukamy najbliższej dostępnej klatki
            for offset in range(1, min(50, total_frames)):  # Ograniczamy szukanie do 50 klatek
                # Próbujemy wcześniejszą klatkę
        ...
        # Konwertujemy BGR -> RGB
        frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        pil_img = Image.fromarray(frame)
        
        # Zmniejszamy rozmiar jeśli za duży
        ...
        if frame_rotation != "Brak":
            ...
            # Automatyczna rotacja dla orientacji pionowej
...
def create_collage_page_optimized(video_path, frame_indices, grid_cols, grid_rows, 
    ...
    # Otwieramy video raz dla całej strony
    ...
    try:
        for i in range(total_cells):
            ...
            # Jeśli nie ma klatki, używamy ostatniej poprawnej
            ...
            # Zwalniamy pamięć po każdej klatce
...
def generate_collage_pages_optimized(video_file, num_pages, grid, page_width, page_height, 
    ...
    # Obliczamy indeksy klatek do pobrania
    ...
    # Generujemy strony jedna po drugiej
...
def save_pages_to_pdf(pages, pdf_path, page_width, page_height):
    ...
    try:
        ...
        for i, page in enumerate(pages):
            ...
            try:
                page.save(temp_path, "JPEG", quality=85)  # Zmniejszona jakość dla oszczędności miejsca
...
def get_user_options():
    ...
    orientation_var = tk.StringVar(value="pionowa")
    ...
    tk.Button(options_window, text="OK", command=finish, bg="lightblue").pack(pady=20)
    ...
    print("  Format(y):", [GRID_FORMATS_LABEL[fmt] for fmt in selected_formats])
    print("  Ilość stron:", num_pages)
    print("  Rotacja:", rotation_var.get())
    
    return orientation_var.get(), selected_formats, num_pages, rotation_var.get()
...
if __name__ == "__main__":
    try:
        ...
        print("Wybierz pliki DAV...")
        file_paths = filedialog.askopenfilenames(title="Wybierz pliki DAV", filetypes=[("Pliki DAV", "*.dav"), ("Wszystkie pliki", "*.*")])
        ...
        orientation, selected_formats, num_pages, frame_rotation = get_user_options()
        ...
        # Przetwarzamy według wybranych formatów
        for fmt_idx, fmt in enumerate(selected_formats):
            ...
            for file_idx, video_file in enumerate(sorted(file_paths)):
                try:
                    ...
                    if fmt_idx == 0:  # Dodajemy do raportu tylko raz
                    ...
                    # Generowanie kolażu z optymalizacją pamięci
                    pages = generate_collage_pages_optimized(
                        video_file, num_pages, fmt, page_width, page_height, orientation, frame_rotation
                    )
                    ...
                    # Zwolnienie pamięci po przetwarzaniu pliku
                    del pages
                    gc.collect()
                    ...
                    print(f"   {e}")
                    traceback.print_exc()
                    # Kontynuujemy z następnym plikiem
                    gc.collect()
        ...
        # Końcowe czyszczenie pamięci
        gc.collect()
    
    # Zapobiegamy natychmiastowemu zamknięciu terminala
